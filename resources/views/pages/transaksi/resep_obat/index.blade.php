...
@section('content')
    <x-adminlte-card title="Resep Obat" theme="dark" icon="fas fa-list-alt">
        
        @include('widget.flash')

        <x-adminlte-modal id="create" title="Create" theme="success"
        icon="fas fa-plus-square" size='lg'>
            <form action="/transaksi/resep_obat" method="post">
            @csrf
            
            <div class="row">
                <x-adminlte-select name="id_kunjungan" label="Kunjungan" fgroup-class="col-md-6" required>
        ...
        <x-adminlte-datatable id="table1" :heads="$heads">
            @foreach($config['data'] as $row)
                <tr>
                    ...
                    <form id="delete{{$row[0]}}" method="POST" action="/transaksi/resep_obat/{{$row[0]}}">
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
                    </form>
